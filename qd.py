...
# Entender o level_count
  ...
  def rank_element(self, rank):
    ...
    while v and (r-l) > 1:
      ...
      if v.child[0]: # se existe filho à esquerda
        u = v.child[0].tree_weight() # peso da arvore do filho à esquerda
        if u < rank:
          if v.child[1]:
            v = v.child[1]
  ...
  def _compress(self, root, C, avail_up):
    ...
    for i in range(2):
      ...
      if child:
        ...
        avail_here = C - root.w
        new_child, move_up_from_child = self._compress(child, C, avail_up + avail_here)
  ...
  def compress(self):
    C = int(self.e*self.W/self.log_u)
    print("Compressing with capacity = ", C)
...
def main():
  ...
  ap.add_argument("-v", "--val", required=True, help="column number to be used to feed the sketch")
  ap.add_argument("-e", "--eps", required=True, help="error bound")
  ap.add_argument("-u", "--univ", nargs='*', required=True, help="universe value + file_path + rank/quant decision + possible query.args")
  ap.add_argument("-i", "--in", required=False, help="optional: file for query.args")
      ...
      for content in contents:
        if (str(args['univ'][2]) == 'rank'):
          ...
          if x < 0 or x > 1:
            print("ERROR: query.args file has invalid values.")
            sys.exit(2)
          # quant = sketck.quant(float(content))
          # print(quant)
      # end for

  else: # query.agrs sem arquivo
    for i in range(len(args['univ'])):
      if i > 2:
        if (str(args['univ'][2]) == 'rank'):
          x = int(args['univ'][i])
...
# eps = 0.1
# n = 1000
# stream = [(random.randrange(univ), random.randrange(max_w)) for i in range(n)]
# true_ranks = univ * [0]
# sketch = Q_digest(eps, univ)
# total_weight = 0
...
# print("counts =", true_ranks)
# for i in range(1,univ):
#   true_ranks[i] += true_ranks[i-1]
# true_ranks.pop(-1)
# true_ranks.insert(0,0)
# print(" ranks =", true_ranks)

# print("QUERIES:")
# for x in range(univ):
#   rank = sketch.rank(x)
#   print("x =",x," true rank =", true_ranks[x], " estimated rank =", rank, " error =", (true_ranks[x]-rank), "(exp max error =", eps*total_weight, ")")
#   element = sketch.rank_element(rank)
#   print("isso eh o q eu quero ",element)
